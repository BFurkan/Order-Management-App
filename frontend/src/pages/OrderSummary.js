...
function OrderSummary() {
  ...
  const fetchOrders = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://10.167.49.200:3007'}/orders`);
  ...
  const handleSaveOrderId = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://10.167.49.200:3007'}/update-order-id`, {
        method: 'POST',
        headers: {
                    ...
                    {filteredOrders.map((order) => (
                      <TableRow key={order.id}>
                        {visibleColumns.productImage && (
                          <TableCell>
                            <img src={`${process.env.REACT_APP_API_URL || 'http://10.167.49.200:3007'}${order.image}`} alt={order.product_name} style={{ width: '100px' }} />
                          </TableCell>
                        )}
